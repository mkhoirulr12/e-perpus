...
<body class="bg-slate-50 text-slate-900">
    ...
    <nav class="bg-primary text-white fixed w-full top-0 z-50 border-b-2 border-gold shadow-md">
        <div class="flex justify-between items-center w-full px-8 py-4 max-w-[1200px] mx-auto">
            ...
            <div class="hidden md:flex items-center space-x-8">
                ...
                <a class="text-gold border-b-2 border-gold pb-1 font-semibold text-sm" href="islami.php">Buku Islami</a>
            </div>
    ...
    <header class="pt-[72px]">
        <div class="bg-gradient-to-br from-primary via-[#4a4a1a] to-primary-dark text-white py-20 relative overflow-hidden">
            <div class="absolute inset-0 opacity-5">
                <div class="absolute top-10 left-10"><span class="material-symbols-outlined text-[200px]">mosque</span></div>
                <div class="absolute bottom-10 right-10"><span class="material-symbols-outlined text-[150px]">menu_book</span></div>
            </div>
            <div class="max-w-[1200px] mx-auto px-8 relative z-10">
                <div class="flex items-center gap-3 mb-4">
                    <span class="material-symbols-outlined text-gold text-4xl">mosque</span>
                    <span class="text-gold text-[10px] font-bold uppercase tracking-[0.3em] bg-gold/10 px-3 py-1 rounded-full">Koleksi Tema</span>
                </div>
                <h1 class="font-heading text-4xl md:text-5xl font-bold mb-4">Buku Islami</h1>
                <p class="text-white/70 max-w-xl text-lg">Koleksi buku-buku Islami yang memperdalam pemahaman agama, akhlak mulia, dan kisah-kisah inspiratif dari ajaran Islam.</p>
    ...
    <main class="max-w-[1200px] mx-auto px-8 py-12 fade-in">
            ...
            <form action="" method="GET" class="flex max-w-xl gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="q" value="<?= htmlspecialchars($search) ?>" 
                           class="w-full border border-slate-200 rounded-xl py-3 pl-12 pr-4 focus:ring-2 focus:ring-amber-400 focus:border-amber-400 outline-none shadow-sm" 
                           placeholder="Cari judul buku islami...">
                </div>
        ...
        <?php if(empty($books)): ?>
            <div class="text-center py-20 border-2 border-dashed border-slate-200 rounded-2xl">
                ...
                <p class="text-slate-400 text-sm">Koleksi buku islami belum tersedia atau kategori belum dibuat.</p>
        ...
        <?php else: ?>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                ...
                <div class="book-card bg-white border border-slate-200 rounded-2xl overflow-hidden cursor-pointer flex flex-col h-full group" onclick="showBookPopup(<?= htmlspecialchars(json_encode($buku)) ?>)">
                    <div class="aspect-[2/3] bg-gradient-to-br from-amber-50 to-slate-100 flex items-center justify-center text-slate-300 border-b border-slate-100 group-hover:from-amber-100 group-hover:to-slate-200 transition-colors overflow-hidden">
                    ...
                    <div class="p-4 flex-1 flex flex-col justify-between">
                        <div>
                            <h4 class="font-bold text-primary text-sm mb-1 line-clamp-2 leading-tight group-hover:text-amber-700 transition-colors"><?= htmlspecialchars($buku['judul_buku']) ?></h4>
